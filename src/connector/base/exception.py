...
class ApiKeyNotFoundError(NotFoundError):
    def __str__(self):
        params = self.args[0]
        return "Commander name/API Key not found {} not found.".format(params['commanderName'])
...
class NoPrivateCommentStored:
    def __str__(self):
        params = self.args[0]
        return "OK, but no private comment stored for this System {}".format(params['systemName'])
